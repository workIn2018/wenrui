...
myApp.onPageInit('shouwen-content',function (page) {
	...
	$.ajax({
		type: "post",
		url: _url+"oa/docmentAccept/get?id="+shouwenId,
		...
		success: function (data) {
			...
			var message = {"docAcceptNum":json.docAcceptNum,"title":json.docSendNum,"deptmentName":json.deptmentName,"acceptType":_acceptType[json.acceptType],"docAcceptTime":_docAcceptTime,"shouwenType":shouwenType,"isZhuihui":isZhuihui,"iclass":"#"+icons[shouwenType]};
			new Vue({
				el: '#shouwenContent',
				data: message
			}); 
...
function getShouwen(type,keywords,pageNo){
	...
	var pageLength = 20;
	if(type==0){
		infoUrl = _url+"workflow/getActivitiList?procDefKey=swgl&pageNo="+pageNo+"&pageSize="+pageLength;
	}else if(type==1){
		infoUrl = _url+"workflow/getUnfinishActivitiList?procDefKey=swgl&pageNo="+pageNo+"&pageSize="+pageLength;
	}else if(type==2){
		infoUrl = _url+"workflow/getFinishedActivitiList?procDefKey=swgl&pageNo="+pageNo+"&pageSize="+pageLength;
	}
	if(pageNo==1){
		$("#tab"+type+" ul").empty();
	}
...
function banli(){
	...
	if(_con==""){
		...
		$.ajax({
			type: "post",
			url: _url+'oa/docmentAccept/optCommitProcess',
			data: _pram,
			dataType: 'jsonp',
	        jsonp: "callback",
	        jsonpCallback:"callback"+new Date().getTime(),
